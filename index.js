...
const Pagination = ({ currentPage, onPaginate, totalPage, batchSize=4, enableFirstPage=true }) => {
  const hasMorePage = totalPage > batchSize;
  const currentBatch = Math.floor((currentPage - 1) * (1 / batchSize));
  const startPage = hasMorePage ? currentBatch * batchSize + 1 : 1;
  const maxPossibleEndPage = Math.min(startPage + batchSize - 1, totalPage);
  ...
  const hasPrevBatch = currentPage > batchSize && hasMorePage;
